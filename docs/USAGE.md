...
- 日志：该日期的学习日志，会按 Markdown 渲染显示，适合查看分级标题、列表、代码片段等结构化笔记。
...
你可以在"设置"页填写自定义 AI 规划 prompt，覆盖默认提示词来控制 AI 如何规划。

### 今日日志（Journal）

独立的日志编辑页面，按日期编辑每日学习日志。日志支持 Markdown 格式，可以在"编辑"和"预览"之间切换，方便把当天学习内容整理成标题、列表、引用、代码块或复盘模板。
...
设置页包含运行设置、ActivityWatch 隐私开关、AI 提示词和后端状态控制。

在"AI 提示词"区域可以分别配置：

- **AI 规划提示词**：控制 AI 如何根据目标、里程碑和最近日志生成今日计划。
- **AI 总结提示词**：控制 AI 如何根据日志、任务、日程和屏幕使用记录生成每日总结。

这两个提示词都可以留空，留空时使用系统默认提示词。填写自定义提示词后，系统仍会自动补充 JSON 输出格式要求，减少 AI 返回内容无法解析的情况。
